...
def format_rga_file_content(
    ...
    {river_file:<69} = Débits de Rivière
    {piezo_file:<69} = Niveaux de Nappe
...
def format_gar_file_content(
        project_description: str,
        basin_description: str,
        user_profile: str,
        execution_mode: str,
        river_obs_weight: int,
        piezo_obs_weight: int,
        river_calc: bool,
        piezo_calc: bool,
        recharge_effective_rain_save: bool,
        river_piezo_save: bool,
        water_balance_save: bool,
        computation_scheme: str,
        spin_up_years: int,
        spin_up_cycles: int,
        data_start_year: int,
        max_calib_iter: int
 ...
 *** Options du Bassin               ***
 ...
 {spin_up_years:>9}=Nombre d'Années démarrage (-n pour générer n année moy fictives de démarrage)
 {spin_up_cycles:>9}=Nombre de cycles de démarrage (déf. = 1)
 {data_start_year:>9}=Date de la première année des données (par ex. 2017)
 ...
 {max_calib_iter:>9}=Nombre maxi. d'itérations pour la calibration (0 = aucune itération, pas de calibrat.)
